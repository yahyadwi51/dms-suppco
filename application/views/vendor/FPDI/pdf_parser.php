        ...
        if ($xrefPos === false) {
            ...
            if (is_array($xrefStreamObjDec) && isset($xrefStreamObjDec[0]) && $xrefStreamObjDec[0] == self::TYPE_OBJDEC) {
                $errorMessage = sprintf(
                    "Dokumen Anda Tidak Support Silahkan Unggah ulang Dokumen Melalui Menu Edit Data"
                );
            
                // Gunakan JavaScript untuk menampilkan pesan kesalahan sebagai pop-up di browser
                echo '<script>alert("' . addslashes($errorMessage) . '");</script>';
                // Redirect pengguna ke halaman "pengelolah_dokumen_dms" tanpa ID
